...
class AbstractController: UIViewController, UITextFieldDelegate, UIGestureRecognizerDelegate {
    ...
    var navFilterButton : UIBarButtonItem {
        let _navSearchButton = UIButton()
        _navSearchButton.setBackgroundImage(UIImage(named: "navFilter"), for: .normal)
        _navSearchButton.frame = CGRect(x: 0, y: 0, width: 25, height: 25)
        _navSearchButton.addTarget(self, action: #selector(filterButtonAction(_:)), for: .touchUpInside)
        return UIBarButtonItem(customView: _navSearchButton)
    }
    ...
    var showNavSearchButton: Bool = false {
        didSet {
            if (showNavSearchButton) {
                self.navigationItem.leftBarButtonItem = navSearchButton
            } else {
                self.navigationItem.leftBarButtonItem = nil
                self.navigationItem.leftBarButtonItems = nil
            }
    ...
    var showNavFilterButton: Bool = false {
        didSet {
            if (showNavFilterButton) {
                self.navigationItem.rightBarButtonItem = navFilterButton
            } else {
                self.navigationItem.rightBarButtonItem = nil
                self.navigationItem.rightBarButtonItems = nil
            }
    ...
    func chatButtonAction( _ sender:AnyObject) {
        ActionShowChat.execute()
    }
    
    
    func filterButtonAction(_ sender:AnyObject){
        ActionShowFilter.execute()
    }
